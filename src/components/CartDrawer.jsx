export default function CartDrawer({ 
  ...
  const handleCheckoutClick = () => {
    if (!currentUser) {
      if (onRequireAuth) onRequireAuth('Sifarişi tamamlamaq və rəsmiləşdirmək üçün daxil olun');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-sm flex justify-end animate-fadeIn">
      ...
      >
        ...
        {/* Note */}
        <div className="px-6 py-2 bg-emerald-50 text-[11px] text-emerald-800 border-b border-emerald-100 flex items-center gap-1.5 font-medium">
          <span>ℹ Səbətdə yalnız birbaşa satılan aqrar məhsullar yer alır.</span>
        </div>
          ...
          {cartItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-2">
              ...
              <p className="text-xs text-gray-500 max-w-xs">
                Gübrələr, toxumlar, tinglər və alətlər bölməsindən məhsullar əlavə edə bilərsiniz.
              </p>
            </div>
          ) : (
            cartItems.map((item) => (

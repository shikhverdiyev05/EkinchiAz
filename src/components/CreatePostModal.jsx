...
export function CreatePostModal({
  ...
  // Reset state synchronously when modal opens or post changes (avoiding useEffect cascading renders)
  // Uses state (not refs) for the "previous value" tracking, since reading
  // ref.current during render is disallowed by the react-hooks/refs rule.
  const [prevOpen, setPrevOpen] = useState(false);
  const [prevPostId, setPrevPostId] = useState(existingPost?.id);
  ...
  if (isOpen && !prevOpen) {
    ...
    setTagInput('');
    setPrevOpen(true);
    setPrevPostId(existingPost?.id);
  } else if (isOpen && existingPost?.id !== prevPostId) {
    setDescription(existingPost?.description || '');
    setTags(Array.isArray(existingPost?.tags) ? [...existingPost.tags] : []);
    setImages(Array.isArray(existingPost?.images) ? [...existingPost.images] : []);
    setTagInput('');
  ...
  // handleClose must be defined BEFORE the useEffects below that reference it
  ...
  const handleSubmit = async (e) => {
    ...
    if (saveError) {
      console.error('Post submit error:', saveError);
      onShowToast?.('Paylaşım zamanı xəta baş verdi');
      return; // save failed — keep the modal open so the user can retry
    }

    // Save succeeded. Each of these runs independently so that an error in
    // a parent-provided callback (onSuccess) can never stop the modal from
    // closing — previously all three shared one try/catch, so a throw from
    // onSuccess/dispatchEvent would skip onClose() and leave the modal open
    // even though the post had already been saved.
    try { onSuccess?.(); } catch (err) { console.error('onSuccess handler error:', err); }
    try { window.dispatchEvent(new Event('refreshPosts')); } catch (err) { console.error('refreshPosts dispatch error:', err); }
    onClose?.();
  };
    ...
    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
      ...
      <div className="px-5 sm:px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center justify-end gap-3 shrink-0">
        <button type="button" onClick={handleClose} disabled={submitting || uploadingImages}
          className="px-5 py-2.5 rounded-xl font-bold text-sm text-gray-600 hover:bg-gray-200 transition disabled:opacity-50">
          Ləğv et
        </button>
        <button type="submit" disabled={submitting || uploadingImages || !description.trim()}
          className="px-7 py-2.5 rounded-xl font-bold text-sm bg-emerald-600 hover:bg-emerald-700 text-white shadow-md transition disabled:opacity-50 flex items-center gap-2">
